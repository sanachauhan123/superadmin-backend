...
router.get('/',verifyRestaurant, async (req, res) => {
  try {
    const orders = await Completeorder.find().sort({ createdAt: -1 });
...
router.put('/:id', verifyRestaurant, async (req, res) => {
  try {
    ...
    const result = await Completeorder.findOneAndUpdate(
...
router.put('/:id/status', verifyRestaurant, async (req, res) => {
  try {
    ...
    await Completeorder.findOneAndUpdate(
      { _id: req.params.id, restaurantId: req.restaurantId },
      { status }
    );
...
router.delete('/:id', verifyRestaurant, async (req, res) => {
  try {
    await Completeorder.findOneAndDelete({ _id: req.params.id, restaurantId: req.restaurantId });
...
router.get('/:id/invoice', verifyRestaurant, async (req, res) => {
  try {
    const order = await Completeorder.findOne({
      _id: req.params.id,
      restaurantId: req.restaurantId
    });
